...
void Computation::toDot(std::fstream& dotFile){
  ...
  for(int i=0; i<getNumStmts(); i++){
       ...
       for(const auto& data_read : getStmt(i)->getDataReads()){
            if(!(std::count(data_spaces.begin(), data_spaces.end(), data_read.first))){
                dotFile << "\t\t" << "subgraph cluster_dataspace" << data_read.first << "{ \n"
                        << "\t\t\t" << "style = filled; \n"
                        << "\t\t\t" << "color = lightgrey; \n"
                        << "\t\t\t" << "label=\" \"; \n"
                        << "\t\t\t" << data_read.first << "[label=\"" << data_read.first << "[] \"] [shape=box][style=filled][color=lightgrey];\n"
                        << "\t\t\t" << "}\n";
                                   
                data_spaces.push_back(data_read.first);
             }
             dotFile <<"\t\t" << data_read.first <<  "->" << "S" << i << "\n";
        }

        for(const auto& data_write : getStmt(i)->getDataWrites()){
            if(!(std::count(data_spaces.begin(), data_spaces.end(), data_write.first))) {
                 dotFile << "\t\t" << "subgraph cluster_dataspace" << data_write.first << "{ \n"
                         << "\t\t\t" << "style = filled; \n"
                         << "\t\t\t" << "color = lightgrey; \n"
                         << "\t\t\t" << "label= \" \"; \n"
                         << "\t\t\t" << data_write.first << "[label=\"" << data_write.first << "[] \"] [shape=box][style=filled][color=lightgrey];\n"
                         << "\t\t\t" << "}\n";
                                     
                         data_spaces.push_back(data_write.first);
               }
            dotFile <<"\t\t" << "S" << i << "->" << data_write.first << "\n";
